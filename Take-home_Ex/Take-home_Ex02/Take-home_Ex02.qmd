...
This project focuses on the development of a robust predictive framework designed to identify and mitigate customer attrition within the Colombian fintech ecosystem. Leveraging the **COFINFAD dataset**—a comprehensive longitudinal record of 48,723 customers over the 2023 fiscal year—the analysis integrates qualitative customer sentiment with quantitative transactional behavior. By synthesizing over 3.1 million raw transaction logs into high-fidelity behavioral metrics, the project establishes a prescriptive, data-driven strategy to optimize proactive customer retention and preserve long-term institutional value.

## 2 Install and Load Packages

In this project, we utilize twelve R packages to facilitate data import, manipulation, and advanced visualization:
...
```{r}
pacman::p_load(
  tidyverse, tidymodels, naniar, skimr, knitr, 
  kableExtra, patchwork, corrplot, ranger, 
  xgboost, vip, car
)
```
...
### 5.1 Justification for Model Development

While the COFINFAD dataset includes a pre-calculated churn probability, this project develops a **de novo predictive engine** using ensemble learning for three strategic reasons:

-   **Transition from Static to Dynamic Analysis:** The original probability column is a static historical record that cannot respond to changes in input data. A custom model is required to power the **interactive simulation environment** in the dashboard, enabling real-time "What-If" adjustments to customer features to observe how risk scores fluctuate.

<!-- -->

-   **Model Interpretability:** To move beyond the "Black Box" nature of the existing data, the framework requires an underlying model object to calculate feature contributions. This allows for the generation of diagnostic visuals that explain the specific drivers behind a customer's risk—a functionality unattainable with a standalone column of static values.

-   **Algorithmic Validation (Benchmarking):** In the absence of binary churn outcomes, the provided probability serves as a **Proxy Ground Truth**. By training optimized models on raw transactional logs, this study seeks to benchmark the ability of ensemble methods to reconstruct these risk scores. A low **Root Mean Square Error (RMSE)** on hold-out data validates that the custom engine has successfully captured the fintech's internal risk logic, ensuring the dashboard’s simulations are mathematically reliable.

### 5.2 Selection of Algorithms: Random Forest and XGBoost

For the predictive modeling phase, **Random Forest** and **XGBoost** were selected due to their superior ability to handle the non-linear relationships and complex feature interactions inherent in financial behavioral data.

-   **Random Forest** provides a robust, ensemble-based baseline that reduces variance by averaging multiple decision trees, making it highly resistant to overfitting and stable across diverse customer segments.

-   **XGBoost (Extreme Gradient Boosting)** was selected for its iterative optimization process, which minimizes residual errors and typically yields higher precision in identifying subtle, high-risk churn signals that simpler models might overlook.

Both models are natively capable of handling the mixed data types present in the COFINFAD dataset—ranging from categorical sentiment scores to continuous financial metrics—without requiring the strict linearity assumptions of traditional regression models.

### 5.3 Create the Preprocessing Recipe
...
### 5.4 Tune and Fit the Models

#### 5.4.1 The Tuning Phase

##### 5.4.1.1 Set Up Cross-Validation Folds
...
##### 5.4.1.2 Tag Hyperparameters for Tuning
...
##### 5.4.1.3 Build the Tuning Workflows
...
#### 5.4.2 The Finalizing and Fitting phase
...
### 5.5 Model Performance and Comparison
...
ggplot(model_comparison, aes(x = model, y = .estimate, fill = model)) +
  geom_col(width = 0.5, show.legend = FALSE) + # Narrower bars for a cleaner look
  # Add labels on top of bars
  geom_text(aes(label = round(.estimate, 4)), vjust = -0.7, size = 3.2, color = "gray20") +
  # Split by metric
  ...
  scale_fill_manual(values = c("Random Forest" = "#4A5568", "XGBoost" = "#A0AEC0")) +
  # Visual refinements
  labs(
  ...
  theme(
    strip.text = element_text(face = "bold", color = "gray10"),
  ...
  # Ensure labels don't get cut off at the top
  expand_limits(y = model_comparison$.estimate * 1.1)
...
The minimal error values recorded—specifically an **MAE of 0.0479** and an **RMSE of 0.0593**—collectively validate the **Random Forest** model as a highly reliable **behavioral proxy** for the original fintech risk logic. Because the average deviation from the baseline probability is less than 5%, the model demonstrates sufficient fidelity to serve as a digital twin. This statistical alignment ensures that any prescriptive insights or "What-If" simulations generated within the dashboard are grounded in a mathematically verified reconstruction of the institution's existing risk architecture.

### 5.6 Variable Importance (Random Forest)
