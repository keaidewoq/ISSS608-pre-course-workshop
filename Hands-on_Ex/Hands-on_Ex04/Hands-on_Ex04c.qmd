...
-   The 95% confidence intervals are computed by using the formula mean+/-1.96\*se.
...
### 3.3 Visualizing the uncertainty of a point estimates with interactive error bars
...
exam %>%
  ggplot(aes(x = RACE, y = MATHS)) +
  # Adding the two interval levels here
  stat_pointinterval(
    .width = c(0.95, 0.99), 
    .point = mean, 
    .interval = qi,
  ...
  labs(
    title = "Visualising confidence intervals of mean math score",
    subtitle = "95% (thick) and 99% (thin) Confidence Intervals"
  )
```
